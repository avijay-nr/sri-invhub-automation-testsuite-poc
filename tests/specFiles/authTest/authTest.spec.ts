...
test(`TC-AUTH-001: Valid Login with SSO - ${appConfig.envName} @auth_TC0001`, async ({ page }) => {
...
test(`TC-AUTH-002: Invalid Login - Wrong OTP - ${appConfig.envName} @auth_TC0002`, async ({ page }) => {
  const authLoginDef = new AuthLoginDef(page);
...
test(`TC-AUTH-003: Session Timeout - ${appConfig.envName} @auth_TC0003`, async ({ page }) => {
  const loginDef = new LoginDef(page);
  const authLoginDef = new AuthLoginDef(page);
...
test.describe('TC-AUTH-004 - isolated logout context', () => {
  ...
  test(`TC-AUTH-004: Logout - ${appConfig.envName} @auth_TC0004`, async ({ page }) => {
    const loginDef = new LoginDef(page);
    const authLoginDef = new AuthLoginDef(page);
...
// ✅ Uses loginIfNeeded() — no OTP needed, just needs to be logged in
test(`TC-AUTH-005: Role-Based Access Control - Verify Admin Access - ${appConfig.envName} @auth_TC0005`, async ({ page }) => {
  const loginDef = new LoginDef(page);
  const authLoginDef = new AuthLoginDef(page);
